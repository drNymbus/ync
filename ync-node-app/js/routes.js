...
const execute_list_insert = async (client, ids, )
...
const store_post = async (req, res, client) => {
    ...
    if (req.query.basket === true) {
        let ids = req.query.id.split(','); // transform query into cql query ready set values
        for (let i in ids) ids[i] = `'${ids[i]}'`;
        await client.execute(utils.basket.add_item, [ids, cookie]); // update basket with new item.s
        ...
        await client.execute(utils.session.update, [Date.now(), cookie]); // update session
        res.status(200).json({ids: ids}); // send ids inserted successfully
    }
};
exports.store_post = store_post;

const store_delete = async (req, res) => {
    ...
    if (req.query.basket === true) {
        let ids = req.query.id.split(','); // transform query into cql query ready set values
        for (let i in ids) ids[i] = `'${ids[i]}'`;
        await client.execute(utils.basket.remove_item, [ids, cookie]); // update basket with new item.s
